    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 overflow-x-auto pb-6">
      {STAGES.map((stage) => {
        ...
        return (
          <div
            key={stage.id}
            className="flex flex-col bg-slate-100/70 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800/80 min-w-[280px] p-3 shadow-xs dark:shadow-lg transition-colors"
          >
            {/* Column Header */}
            <div className={`border-t-2 ${stage.color} pt-2.5 pb-2 px-1 mb-2`}>
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-slate-800 dark:text-slate-200 tracking-wide">{stage.title}</h3>
              ...
              {stageLeads.length === 0 ? (
                ...
                stageLeads.map((lead) => (
                  <div
                    ...
                    className="bg-white dark:bg-slate-800/80 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-200/90 dark:border-slate-700/60 hover:border-slate-300 dark:hover:border-slate-600 rounded-xl p-3.5 shadow-xs dark:shadow-md hover:shadow-indigo-500/10 transition-all cursor-pointer group"
