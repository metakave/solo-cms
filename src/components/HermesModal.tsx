...
export const HermesModal: React.FC<HermesModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#0e1626] border border-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl p-6 space-y-5 animate-in fade-in zoom-in-95">
        {/* Header */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-300 border border-purple-500/30">
                  Ready
                </span>
              </h2>
        ...
        {/* Secret Key Strip */}
        <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl flex items-center justify-between gap-3 text-xs">
          <div>
            <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
              Hermes Secret Key (Header: x-hermes-key)
            </div>
            <div className="font-mono text-indigo-300 mt-0.5">{hermesKey}</div>
          </div>
          <button
            onClick={() => handleCopy(hermesKey, setCopiedKey)}
            className="flex items-center gap-1 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg font-medium transition-all"
          >
        ...
        {/* Live Test Trigger */}
        <div className="bg-purple-950/20 border border-purple-900/40 rounded-xl p-4 space-y-2.5">
          ...
          {testStatus && (
            <div className="text-xs p-2.5 rounded-lg bg-slate-950 border border-purple-900/60 text-slate-200 font-mono">
              {testStatus}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <pre className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-[10px] font-mono text-slate-400 max-h-40 overflow-y-auto">
